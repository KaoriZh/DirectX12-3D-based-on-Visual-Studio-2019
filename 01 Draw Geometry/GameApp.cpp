...
private:
	...
	virtual void UpdateKeyboard() override; // 重写键盘监听
...
void GameApp::OnResize()
{
    ...
    XMMATRIX P = XMMatrixPerspectiveFovLH(DirectX::XM_PIDIV4, AspectRatio(), mNearZ, mFarZ);
...
void GameApp::UpdateMouse() {
	UpdateMouseDefault();
}

void GameApp::UpdateKeyboard() {
	// 获取键盘状态
	...
	if (keyState.IsKeyDown(Keyboard::Space)) {
		ResetCameraState();
	} else if (keyState.IsKeyDown(Keyboard::X)) {
		mIsWireframe = 1 - mIsWireframe;
	}
}

void GameApp::UpdateFrameResource() {
...
void GameApp::UpdateMainPassCB(const GameTimer& gt) {
	XMMATRIX view = XMMatrixLookAtLH(mCameraPos, mTargetPos, mUpDir);
...
void GameApp::Draw(const GameTimer& gt)
{
	...
	XMVECTORF32 bgColor = { { { 0.121568627f, 0.121568627f, 0.121568627f, 1.000000000f } } };
	mCommandList->ClearRenderTargetView(CurrentBackBufferView(), bgColor, 0, nullptr);
...
void GameApp::BuildRendedrItems() {
