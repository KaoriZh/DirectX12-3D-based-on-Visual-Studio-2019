...
:	mhAppInst(hInstance), mScissorRect(), mScreenViewport()
{
...
void D3DApp::InitMouseDefault() {
	mMouse->SetWindow(mhMainWnd);
	mMouse->SetMode(DirectX::Mouse::MODE_ABSOLUTE);
}

void D3DApp::UpdateMouseDefault() {
	// 获取鼠标状态
	...
	if (mouseState.rightButton == true && mMouseTracker.rightButton == mMouseTracker.HELD) {
		mTheta -= (mouseState.x - lastMouseState.x) * 0.01f;
		mPhi -= (mouseState.y - lastMouseState.y) * 0.01f;
	}
	mCameraPos = MathHelper::SphericalToCartesian(mRadius, mTheta, mPhi) + mTargetPos;
	// 拖拽视图
	if (mouseState.middleButton == true && mMouseTracker.middleButton == mMouseTracker.HELD) {
	...
	float temp = (mouseState.scrollWheelValue - lastScrollValue) * 0.01f;
	mRadius -= temp;
	// 鼠标速度适配
	mMouseSpeed -= temp;
...
void D3DApp::UpdateKeyboardDefault() {
	// 获取键盘状态
...
void D3DApp::ResetCameraState() {
	mTheta = DirectX::XM_PIDIV4;
	mPhi = DirectX::XM_PIDIV4;
	mRadius = 5.0f;
	mCameraPos = MathHelper::SphericalToCartesian(mRadius, mTheta, mPhi);
